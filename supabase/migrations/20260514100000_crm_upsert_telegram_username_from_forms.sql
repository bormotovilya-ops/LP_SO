-- Заполнять telegram_username при апсерте без Telegram ID (анкеты с сайта / квиз),
-- если передан валидный p_telegram_username и в строке контакта ник ещё пустой.
-- Приоритет у пары telegram_id + username из бота (как раньше).
...
COMMENT ON FUNCTION public.crm_upsert_contact(
  ...
  'Upsert лида по телефону / email / Telegram / resolve id; несколько совпадений сливает в один контакт. p_telegram_username — без @ из бота или с форм (запись только если без telegram_id ник в БД был пуст).';
