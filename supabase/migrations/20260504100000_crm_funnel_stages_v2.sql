...
-- Уникальный sort_order: сначала сдвиг, затем расстановка активных этапов.
update public.crm_pipeline_stages
set sort_order = sort_order + 10000000
where sort_order < 10000000;

-- Новый этап «Интерес подтверждён» (раньше частично покрывался diagnostic_requested / quiz_completed).
...
-- Перенос текущих контактов с устаревших этапов на новые коды.
...
-- Устаревшие автоматические этапы скрываем из воронки (история по UUID сохраняется).
update public.crm_pipeline_stages
set
  is_active = false,
  updated_at = timezone('utc', now())
...
-- Переименование кодов с сохранением id (FK в истории этапов).
update public.crm_pipeline_stages
set
...
-- Подписи и порядок активной воронки (блоки — только в названиях для людей).
update public.crm_pipeline_stages
...
-- Новые лиды всегда стартуют с «Новый лид»; дальнейшие этапы — вручную или через события (crm_change_stage).
